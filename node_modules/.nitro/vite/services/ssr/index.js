...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./assets/server-DVhiItC6.js").then((m) => m.default ?? m);
	return serverEntryPromise;
}
async function getRegistrationsCollection() {
	if (!MONGODB_URI) throw new Error("Missing MONGODB_URI in production.");
	if (!mongoClient) {
		mongoClient = new MongoClient(MONGODB_URI, { serverSelectionTimeoutMS: 5e3 });
